...
import "../../../../../components/css/subject.css";
...
const StudentDetails = () => {
  ...
  useEffect(() => {
    ...
    // Fetch staff list
    fetch('/api/staff_master')
      .then(res => res.json())
      .then(data => {
        setStaff(data || []);
      })
      .catch(err => {
        console.error('Error fetching staff master:', err);
        setStaff([]);
      });
  }, []);
    ...
    try {
      ...
      try {
        ...
        if (eduRes.ok) {
          ...
          // No education details found - normal for new students
        } else {
          // Error fetching education details
        }
      } catch (err) {
        // Continuing with empty education data
      }
      ...
      setForm({
        ...
        classTeacher1: selectedStudent.Class_Teacher_1 ?? '',
        classTeacher2: selectedStudent.Class_Teacher_2 ?? '',
      ...
      if (educationData) {
        ...
        // No education data found - will use default sections
      }
  ...
  const handleExportTemplate = useCallback(async (isTemplate = false) => {
    try {
      const response = await fetch(`/api/studentMaster/bulk/export-template?template=${isTemplate}`);
      ...
      a.href = url;
      a.download = isTemplate ? 'student_template.xlsx' : 'students_data.xlsx';
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      a.remove();
      toast.success(isTemplate ? 'Template exported successfully' : 'Student data exported successfully');
    } catch (err) {
      console.error('Export error:', err);
      toast.error(isTemplate ? 'Failed to export template' : 'Failed to export student data');
    }
  }, []);

  const handleConfirmBulkImport = useCallback(async (students, toastId) => {
    try {
      toast.info('Importing data...', { autoClose: false, toastId: 'importing' });
      const response = await fetch('/api/studentMaster/bulk/confirm-import', {
        method: 'POST',
      ...
      if (result.success) {
        if (result.importedCount > 0) {
          toast.success(`Successfully imported ${result.importedCount} students! (Inserted: ${result.insertedCount || 0}, Updated: ${result.updatedCount || 0})`);
          if (result.failedCount > 0) {
            toast.warning(`${result.failedCount} rows failed. Check console for details.`);
            console.error('Import failures:', result.errors);
          }
          setRefreshTable(prev => prev + 1);
          setShowTable(true);
        } else {
          toast.error('No students were imported. See console for error details.');
          console.error('Import failed - Detailed Errors:', result.errors);
          if (result.errors && result.errors.length > 0) {
              const firstErr = result.errors[0];
              toast.error(`First error: ${firstErr.error} (Student: ${firstErr.appNo})`, { duration: 5000 });
          }
        }
      }
    } catch (err) {
      ...
      toast.error(`Import confirmation failed: ${err.message}`);
    }
  }, []);
  ...
  const handleImportExcel = useCallback(async (e) => {
    ...
    setIsImporting(true);
    try {
      const response = await fetch('/api/studentMaster/bulk/import-excel', {
        method: 'POST',
        body: formData,
      });
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Failed to process Excel');
      }
      ...
      toast((t) => (
        <div>
          <p className="mb-2">Ready to import <strong>{result.count}</strong> students?</p>
          <div className="d-flex gap-2">
            <button
              className="btn btn-primary btn-sm"
              onClick={() => handleConfirmBulkImport(result.fullData, t.id)}
            >
              Confirm Import
            </button>
            <button
    ...
    if (name === 'sslcLanguage1' || name === 'sslcLanguage2') {
      setForm(prev => {
        ...
        if (updated.sslcSubjects && updated.sslcSubjects.length > 0) {
          updated.sslcSubjects = (Array.isArray(updated.sslcSubjects) ? updated.sslcSubjects : []).map((subject, idx) => {
    ...
    if (name === 'hscLanguage1' || name === 'hscLanguage2') {
      setForm(prev => {
        ...
        if (updated.hscSubjects && updated.hscSubjects.length > 0) {
          updated.hscSubjects = (Array.isArray(updated.hscSubjects) ? updated.hscSubjects : []).map((subject, idx) => {
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      const masterPayload = {
        ...
        Class_Teacher_1: form.classTeacher1,
        Class_Teacher_2: form.classTeacher2,
        Class: form.class,
        Batch: form.batch,
          ...
          <table className="table table-bordered">
            ...
            <tbody>
              {form.sslcSubjects && Array.isArray(form.sslcSubjects) && form.sslcSubjects.length > 0 ? (
                (Array.isArray(form.sslcSubjects) ? form.sslcSubjects : []).map((subject, index) => (
                  <tr key={index}>
                    <td>
                      <input
                        type="text"
          ...
          <table className="table table-bordered">
            ...
            <tbody>
              {form.itiSubjects && form.itiSubjects.length > 0 ? (
                (Array.isArray(form.itiSubjects) ? form.itiSubjects : []).map((subject, index) => (
                  <tr key={index}>
                    <td>
                      <input
                        type="text"
          ...
          <table className="table table-bordered">
            ...
            <tbody>
              {form.vocationalSubjects && Array.isArray(form.vocationalSubjects) && form.vocationalSubjects.length > 0 ? (
                (Array.isArray(form.vocationalSubjects) ? form.vocationalSubjects : []).map((subject, index) => (
                  <tr key={index}>
                    <td>
                      <input
                        type="text"
      ...
      <div className="row g-3">
        ...
        <div className="col-md-3">
          ...
          <select
            ...
            className="form-select"
          >
            {(Array.isArray(HSC_MAJORS) ? HSC_MAJORS : []).map((major) => (
          ...
          <table className="table table-bordered">
            ...
            <tbody>
              {form.hscSubjects && Array.isArray(form.hscSubjects) && form.hscSubjects.length > 0 ? (
                (Array.isArray(form.hscSubjects) ? form.hscSubjects : []).map((subject, index) => (
                  <tr key={index}>
                    <td>
                      <input
                        type="text"
                ...
                <div className="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-24">
                  ...
                  <div className="d-flex align-items-center gap-2">
                    <div className="dropdown d-inline-block">
                      <button
                        className="btn btn-outline-primary-600 radius-8 px-20 py-11 dropdown-toggle"
                        type="button"
                      ...
                      <ul className="dropdown-menu">
                         <li>
                          <button type="button" className="dropdown-item" onClick={() => handleExportTemplate(true)}>
                            <i className="fas fa-file-download me-2 text-primary"></i> Export Template
                          </button>
                        </li>
                        <li>
                          <button type="button" className="dropdown-item" onClick={() => handleExportTemplate(false)}>
                            <i className="fas fa-file-export me-2 text-info"></i> Export Student Data
                          </button>
                        </li>
                        <li>
                          <button type="button" className="dropdown-item" onClick={() => importFileRef.current.click()}>
                            <i className="fas fa-file-upload me-2"></i> Import Excel
                          </button>
                        </li>
                      </ul>
                      <input
                ...
                <form onSubmit={handleSubmit}>
                            ...
                            {renderFormField('community', 'select', {
                              children: (
                                <>
                                  ...
                                  {(Array.isArray(communityOptions) ? communityOptions : []).map((c) => (
                    ...
                    <div className="tab-content-section">
                      <div className="mb-3">
                        ...
                        <Select
                          ...
                          value={(Array.isArray(EDUCATION_SECTION_OPTIONS) ? EDUCATION_SECTION_OPTIONS : []).filter(opt => educationSections[opt.value])}
                          onChange={selected => {
                          ...
                          <div className="mb-5 pb-4 border-bottom">
                            ...
                            {form.sslcNumberOfAttempts && parseInt(form.sslcNumberOfAttempts) > 0 && Array.isArray(form.sslcExaminationAttempts) && form.sslcExaminationAttempts.length > 0 && (
                              <div className="attempts-container">
                                ...
                                {(Array.isArray(form.sslcExaminationAttempts) ? form.sslcExaminationAttempts : []).map((attempt, index) => {
                          ...
                          <div className="mb-5 pb-4 border-bottom">
                            ...
                            {form.itiNumberOfAttempts && parseInt(form.itiNumberOfAttempts) > 0 && Array.isArray(form.itiExaminationAttempts) && form.itiExaminationAttempts.length > 0 && (
                              <div className="attempts-container">
                                ...
                                {(Array.isArray(form.itiExaminationAttempts) ? form.itiExaminationAttempts : []).map((attempt, index) => {
                                  const safeAttempt = attempt || {};
                                  return (
                          ...
                          <div className="mb-5 pb-4 border-bottom">
                            ...
                            {form.vocationalNumberOfAttempts && parseInt(form.vocationalNumberOfAttempts) > 0 && Array.isArray(form.vocationalExaminationAttempts) && form.vocationalExaminationAttempts.length > 0 && (
                              <div className="attempts-container">
                                ...
                                {(Array.isArray(form.vocationalExaminationAttempts) ? form.vocationalExaminationAttempts : []).map((attempt, index) => {
                                  const safeAttempt = attempt || {};
                                  return (
                          ...
                          <div className="mb-5 pb-4">
                            ...
                            {form.hscNumberOfAttempts && parseInt(form.hscNumberOfAttempts) > 0 && Array.isArray(form.hscExaminationAttempts) && form.hscExaminationAttempts.length > 0 && (
                              <div className="attempts-container">
                                ...
                                {(Array.isArray(form.hscExaminationAttempts) ? form.hscExaminationAttempts : []).map((attempt, index) => {
                                  const safeAttempt = attempt || {};
                                  return (
                  ...
                  {activeTab === 'course' && (
                    <div className="tab-content-section">
                      ...
                      <div className="mb-32 pb-16 border-bottom">
                        ...
                        <div className="row g-3">
                          ...
                          <div className="col-md-3">
                            ...
                            {form.Course_Name && (Array.isArray(departments) ? departments : []).filter(d => d.Course_Name === form.Course_Name).length === 0 && departments.length > 0 && (
                              ...
                              children: (
                                <>
                                  ...
                                  {form.modeOfJoining === 'Lateral'
                                    ? (Array.isArray(semesters) ? semesters : []).filter((sem) => parseInt(sem.Semester) >= 3).map((sem) => (
                            ...
                            {renderFormField('academicYear', 'select', {
                              ...
                              children: (
                                <>
                                  ...
                                  {(Array.isArray(availableAcademicYears) ? availableAcademicYears : []).map((year) => (
                      ...
                      <div className="mb-32 pb-16 border-bottom">
                        <h5 className="fw-semibold mb-3">Class Details</h5>
                        <div className="row g-3">
                          <div className="col-md-3">
                            <label className="form-label">Class Teacher 1</label>
                            <select name="classTeacher1" value={form.classTeacher1 || ''} onChange={handleChange} className="form-select">
                              <option value="">Select Teacher</option>
                              {(Array.isArray(staff) ? staff : []).map((s) => (
                                <option key={s.Staff_ID} value={s.Staff_ID}>{s.Staff_Name} ({s.Staff_ID})</option>
                              ))}
                            </select>
                          </div>
                          <div className="col-md-3">
                            <label className="form-label">Class Teacher 2</label>
                            <select name="classTeacher2" value={form.classTeacher2 || ''} onChange={handleChange} className="form-select">
                              <option value="">Select Teacher</option>
                              {(Array.isArray(staff) ? staff : []).map((s) => (
                                <option key={s.Staff_ID} value={s.Staff_ID}>{s.Staff_Name} ({s.Staff_ID})</option>
                              ))}
                          ...
                          <div className="col-md-3">
                            ...
                            <input type="text" name="rollNumber" value={form.rollNumber || ''} onChange={handleChange} className="form-control" placeholder="Enter roll number" />
                          </div>
                          <div className="col-md-3">
                            <label className="form-label">Regulation</label>
                            <input type="text" name="regulation" value={form.regulation || ''} onChange={handleChange} className="form-control" placeholder="Enter regulation" />
                          </div>
                          <div className="col-md-3">
                            <label className="form-label">Class</label>
                            <input type="text" name="class" value={form.class || ''} onChange={handleChange} className="form-control" placeholder="Enter class" />
                          </div>
                          <div className="col-md-3">
                            <label className="form-label">Batch</label>
                            <input type="text" name="batch" value={form.batch || ''} onChange={handleChange} className="form-control" placeholder="Enter batch" />
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="d-flex justify-content-end gap-3 mt-24 pt-24 border-top border-neutral-200">
                    <button
                      ...
                      onClick={() => navigate('/admin/admission/admission/report', { state: { studentData: form } })}
                      title="View Admission Report"
                    >
                      <i className="fas fa-file-alt me-2"></i> Application Form
                    </button>
                    ...
                    <button type="button" className="btn btn-outline-danger px-20" onClick={handleReset} title="Reset all fields">Reset</button>
                  </div>
                </form>
              </div>
            </div>
